...
def main():
    ...
    # Грешка 1, печатна грешка при дефинирање на функцијата getRandomizedBoard
    mainBoard = getRandomizedBoard()
    revealedBoxes = generateRevealedBoxesData(False)

    firstSelection = None  # stores the (x, y) of the first box clicked.
    num_tries = 0  # Број на обиди на играчот
    max_tries = (BOARDHEIGHT*BOARDWIDTH/2) + 1  # Максимален број на обиди
    ...
    while True:  # main game loop
        ...
        # Проверка дали е достигнат можниот број обиди
        ...
        # Грешка 2, фали повик на функцијата getBoxAtPixel
            ...
            if not revealedBoxes[boxx][boxy] and mouseClicked:
                ...
                if firstSelection == None:  # the current box was the first box clicked
                    ...
                    icon2shape, icon2color = getShapeAndColor(
                        mainBoard, boxx, boxy)
                    num_tries += 1      # Бројач кој брои обиди на играчот
        ...
        pygame.display.update()         # Грешка 3, екранот не се ажурира
        FPSCLOCK.tick(FPS)
...
def splitIntoGroupsOf(groupSize, theList):
    ...
    return result       # Грешка 4, функцијата не враќа ништо


def leftTopCoordsOfBox(boxx, boxy):
    # Convert board coordinates to pixel coordinates
    # Грешка 5, фалат загради во следните два изрази
...
def getShapeAndColor(board, boxx, boxy):
    ...
    # грешка 7, индексите 0 и 1 се поставени на погрешно место
...
def drawBoxCovers(board, boxes, coverage):
    ...
    # Грешка 8, грешна индентација, последните две линии код не припаѓаат во for циклусот
    pygame.display.update()
    FPSCLOCK.tick(FPS)
...
def gameWonAnimation(board):
    ...
    # Се вчитува сликата која се наоѓа во ист фолдер со .ру играта
    image = pygame.image.load('congrats.png')
    # Иницијална позиција на сликата (во координати)
    imagex = 5
    imagey = 5
    # Иницијален правец во кој ќе се придвижува сликата
    direction = 'right'
    # Придвижувањето ќе се одвива 10 секунди
    end = time.time() + 10
    DISPLAYSURF.fill(WHITE)

    # Додека се извршува while циклусот, сликата се придвижува во правец десно->долу->лево->горе->десно
    # Top level if-овите се однесуваат за насоката, а внездените за
    # проверка за тоа дали сликата стигнала до одредена „граница“
    while time.time() < end:
        if direction == 'right':
            imagex += 5
            if imagex == 280:
