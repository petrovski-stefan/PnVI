...
BOARDWIDTH = 8  # Baranje 1
BOARDHEIGHT = 8  # Baranje 1
TILESIZE = 80
WINDOWWIDTH = 1000  # Tuka ja zgolemiv goleminata na prozorecot zatoa sto
WINDOWHEIGHT = 800  # ne se prikazuvase tekstot zaradi pogolemiot broj na polinja

FPS = 60  # Zgolemen e brojot na FPS so cel da se zabrza animacijata pri lizganje na polinjata
BLANK = None
...
game_moves = []  # Tuka ke gi cuvam site akcii vo igrata
...
def main():
    ...
    # Dodadov novo kopce UNDO
    UNDO_SURF, UNDO_RECT = makeText(
        "UNDO", TEXTCOLOR, TILECOLOR, WINDOWWIDTH - 120, WINDOWHEIGHT - 150
    )

    mainBoard, solutionSeq = generateNewPuzzle(64)
    SOLVEDBOARD = (
        getStartingBoard()
    )  # a solved board is the same as  board in a start state.
...
# Funkcija za handle na undo akcijata
def drawUndo(board):
    # Game_moves ima oblik na pr. right, left, up, left ...
    if len(game_moves) > 0:
        # Proveruvam dali listata ne e prazna za da ne mi dade exception
        last_move = game_moves.pop()

        # Dictionary od sprotivni dvizenja
        opp_moves = {"right": "left", "left": "right", "up": "down", "down": "up"}
        ...
        makeMove(board, opp_moves[last_move], True)
...
def makeMove(board, move, isUndo=False):
